...
import { 
  ...
  Info, ExternalLink, GraduationCap, ArrowRight, Network, Eye,
  Wind
} from 'lucide-react';
...
const DotGridLayer = () => {
  ...
  useEffect(() => {
    ...
    // Physics Constants
    const DOT_SPACING = 28; // Matches CSS Grid background-size
    const DOT_SIZE = 1.9; 
    const MOUSE_RADIUS = 120; 
    const RETURN_SPEED = 0.5; 
    const DISPLACE_STRENGTH = 0.5; 
    const BASE_COLOR = 'rgba(39, 30, 55, 0.26)';
    ...
    <div className="absolute inset-0 z-0 pointer-events-none bg-gradient-to-br from-white via-slate-50 to-slate-200 overflow-hidden">
        ...
        <div 
            ...
            style={{
                ...
                maskImage: 'radial-gradient(ellipse at center, black 40%, transparent 100%)' // Fade edges
            }} 
        />
...
const generateData = () => {
  ...
  const extraConcepts = [
    { id: 'Data Structures', group: 1 }, { id: 'Algorithms', group: 1 }, { id: 'Microservices', group: 1 },
    { id: 'Docker & K8s', group: 1 }, { id: 'Cryptography', group: 1 }, { id: 'Network Security', group: 1 },
    { id: 'Gradient Descent', group: 2 }, { id: 'Matrices', group: 2 }, { id: 'Bayesian Theorem', group: 2 },
    { id: 'Neural Networks', group: 3 }, { id: 'Loss Optimization', group: 3 }, { id: 'Transformers', group: 3 },
    { id: 'LLMs', group: 3 }, { id: 'Robotics', group: 3 }, { id: 'Reinforcement Learning', group: 3 },
    { id: 'Bias & Fairness', group: 4 }, { id: 'Tech Policy', group: 4 }, { id: 'Market Analysis', group: 4 }
  ].map(n => enrich({ ...n, type: 'concept', val: 4 }));

  const nodes = [...baseNodes, ...extraConcepts];
  
  const links = [
    { source: 'CompSci Major', target: 'Web Development' }, { source: 'Web Development', target: 'React Frameworks' },
    { source: 'Web Development', target: 'Backend Systems' }, { source: 'CompSci Major', target: 'Cloud Computing' },
    { source: 'CompSci Major', target: 'Cybersecurity' }, 
    { source: 'Applied Math', target: 'Calculus' }, { source: 'Applied Math', target: 'Linear Algebra' },
    { source: 'Applied Math', target: 'Statistics' }, { source: 'Calculus', target: 'Gradient Descent' },
    { source: 'Linear Algebra', target: 'Matrices' }, { source: 'Statistics', target: 'Bayesian Theorem' },
    { source: 'AI Specialization', target: 'Intro to AI' }, { source: 'AI Specialization', target: 'Machine Learning' },
    { source: 'Machine Learning', target: 'Neural Networks' }, { source: 'Machine Learning', target: 'Loss Optimization' },
    { source: 'AI Specialization', target: 'Computer Vision' }, { source: 'AI Specialization', target: 'NLP' },
    { source: 'NLP', target: 'Transformers' }, { source: 'NLP', target: 'LLMs' },
    { source: 'Business Minor', target: 'Entrepreneurship' }, { source: 'Entrepreneurship', target: 'Economics' },
    { source: 'Business Minor', target: 'AI Ethics' }, { source: 'AI Ethics', target: 'Bias & Fairness' },
  ...
  extraConcepts.forEach(node => {
    if (!links.find(l => l.target === node.id || l.source === node.id)) {
        let target;
...
const { nodes: GEN_NODES, links: GEN_LINKS } = generateData();
...
const App = () => {
  const [nodes] = useState(GEN_NODES);
  const [links] = useState(GEN_LINKS);
  ...
  const COLORS = [
    '#FF3B30', '#30D158', '#0A84FF', '#BF5AF2', '#FF9F0A', '#64D2FF'
  ];
  const getColor = (group) => COLORS[group] || '#8E8E93';

  // Label Visibility Logic
  const getNodeVisibilityThreshold = (d) => {
      let factor = 1.0;
  ...
  const updateLabels = () => {
    ...
    gRef.current.selectAll('text').transition().duration(200)
  ...
  useEffect(() => {
    ...
    defs.append('marker').attr('id', 'arrow').attr('viewBox', '0 -5 10 10')
      ...
      .attr('orient', 'auto').append('path').attr('d', 'M0,-5L10,0L0,5').attr('fill', 'rgba(255,255,255,0.3)');

    COLORS.forEach((color, i) => {
        defs.append('marker').attr('id', `arrow-colored-${i}`).attr('viewBox', '0 -5 10 10')
        ...
        const grad = defs.append('radialGradient').attr('id', `glow-grad-${i}`).attr('cx', '50%').attr('cy', '50%').attr('r', '50%');
    ...
    svg.call(zoom).call(zoom.transform, d3.zoomIdentity.translate(width/2, height/2).scale(0.6));
    ...
    const linkHitArea = g.append('g').selectAll('line').data(links).join('line')
      ...
      .on('click', (e) => { e.stopPropagation(); setActiveNode(null); });
    ...
    node.on('mouseenter', function(event, d) {
        ...
        g.selectAll('.visible-link')
            .classed('link-hovered', l => l.source.id === d.id || l.target.id === d.id)
            .style('stroke', l => (l.source.id === d.id || l.target.id === d.id) ? getColor(l.source.group) : null)
            .attr('marker-end', l => (l.source.id === d.id || l.target.id === d.id) ? `url(#arrow-colored-${l.source.group % 6})` : 'url(#arrow)');
    ...
    const ticker = d3.timer((elapsed) => {
        ...
        linkHitArea
            ...
            .attr('y1', d => d.source.visualY || d.source.y) // Fallback if init
            .attr('x2', d => d.target.x)
            .attr('y2', d => d.target.visualY || d.target.y);

        link
            .attr('x1', d => d.source.x)
            .attr('y1', d => d.source.visualY || d.source.y)
            .attr('x2', d => d.target.x)
            .attr('y2', d => d.target.visualY || d.target.y);
    });
  ...
  useEffect(() => {
     ...
     if (!activeNode) {
         ...
         g.selectAll('.node-group').classed('node-active', false)
            .select('.node-inner-content').select('.node-glow').attr('r', d => d.val + 10);
     ...
     g.selectAll('.node-group').classed('node-active', n => n.id === activeNode.id || neighbors.has(n.id));

     g.selectAll('.visible-link')
        .classed('link-active', l => l.source.id === activeNode.id || l.target.id === activeNode.id)
        .style('stroke', l => (l.source.id === activeNode.id || l.target.id === activeNode.id) ? getColor(l.source.group) : null)
        .attr('marker-end', l => (l.source.id === activeNode.id || l.target.id === activeNode.id) ? `url(#arrow-colored-${l.source.group % 6})` : 'url(#arrow)');
        
     g.selectAll('.node-group').filter(d => d.id === activeNode.id)
       .select('.node-inner-content').select('.node-glow')
       .transition().duration(600).attr('r', activeNode.val * 4.5);
  ...
  const resetView = () => {
    ...
    if (svgRef.current && zoomRef.current) {
        d3.select(svgRef.current).transition().duration(1000).call(zoomRef.current.transform, d3.zoomIdentity.translate(0,0).scale(0.6));
    }
  };
